...
phrases=[
    'Ваш мем, сэр <:lambert:595522325152137220>',\
    'Твой мем, бомжара <:bonjour_epta:605476358407323658>',\
    'Хотел мема - подавись',\
    'Ставь кулсторибоба если не смешно <:coolstorybob:440201373410131978>',\
    'Мемас?',\
    'Кто заказывал мемас?',\
    'Memes is three hundred bucks <:MRCO:589181415300792320>',\
    'Ставь лукаса если смешно',\
    'Я тебя ненавижу',\
    'Если тебе смешно значит мем смешной woof',\
    'Кто следующий попросит мем тот еблан <:gravi:589177100125208617>',\
    'АЛЛО!',\
    'Этот мем скинул полный даун конечно',\
    'Жиза?',\
    'Так то у меня свой бизнес, а мемы чисто для души отправляю',\
    '<:coolstorybob:440201373410131978>',\
    '<:coolstorybob:440201373410131978>',\
    'Никак вы, блять, не научитесь <:geraltsleep:590156064654098433>',\
    '','','','','','','','','','',''\
]
phrasesthree=[
    'Ваш мем оказался предателем <:danceAmongUs:758389940664270938>',\
    'Через 5 минут сумка',\
    'Я ведь лучше чем дино? <:sadcat:589174859087478784> ',\
    'Я знаю что вы тут андерграунд, и я вас уважаю.',\
    'Ради бога, напишите хоть кто-нибудь что мем говно',\
    'НУ ЗАЧЕМ ТЕБЕ ТРИ МЕМА!!!!!!!!',\
    'Ведется набор в ботнет на серьезное обеспечение, в планах захватить мир, гс в пм',\
    'Ты вообще в курсе что люди должны РАБОТАТЬ?',\
    '!рулетка',\
    'Да ты добился хоть чего-нибудь чтоб заслужить эти мемы:',\
    'Если ниже будет рефейс, то ты гей','АЛЛО!',\
    'Три мема. В три раза больше ненависти ко мне...',\
    'Три мема. В три раза больше боянов...',\
    'Никак вы, блять, не научитесь <:geraltsleep:590156064654098433>',\
    '','','','','','','','','','',''\
]
emojis=[]
...
@bot.slash_commands(name='мем', description='Получи свой мем!')
async def memsCommand(ctx, count: int):
    count = min(max(1, count), 3)
    for item in os.walk('%s' % (images_path) ):
        images=list(pic for pic in item[2])
        image=random.choice(images)
        phrase=random.choice(phrases)
        yield from await ctx.send(phrase, file=discord.File(images_path + image))
